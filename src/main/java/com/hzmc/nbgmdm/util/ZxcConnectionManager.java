 ...
 * 
 */
public class ZxcConnectionManager {

	private static ZxcConnectionManager	instance	= null;
	private BasicDataSource 			ds;
	private static final Logger			logger		= Logger.getLogger(ZxcConnectionManager.class);

	/**
	 * 设置连接参数 InitialPoolSize:10 MaxPoolSize:30
	 */
	private ZxcConnectionManager() {
		try {
			Property property = new Property();
			property.getJDBCProperties("zxc.properties");
...
/*			ds.setJdbcUrl(Property.jdbcUrl);
	 ...
	 * @return instance
	 */
	public static final ZxcConnectionManager getInstance() {
		if (instance == null) {
			try {
				instance = new ZxcConnectionManager();
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("ZxcConnectionManager.getInstance():" + e.getMessage());
			}
		}
		return instance;
	}
	...
	public synchronized final Connection getConnection() {
		try {
			...
			e.printStackTrace();
			logger.error("ZxcConnectionManager.getConnection():" + e.getMessage());
		}
		return null;
	}
}
